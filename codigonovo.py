...
tipo_carro = input("\nDigite o tipo de carro (Híbrido, Sedan, SUV, Compacto, Esportivo): ")
if tipo_carro:
preferencias_usuario["tipo"] = tipo_carro.lower()
consumo_carro = input("\nDigite o consumo mínimo desejado (ou deixe em branco para
qualquer valor): ")
if consumo_carro:

preferencias_usuario["consumo"] = float(consumo_carro)
capacidade_carro = input("\nDigite a capacidade mínima desejada (ou deixe em branco para
qualquer valor): ")
if capacidade_carro:
preferencias_usuario["capacidade"] = int(capacidade_carro)
aluguel_max_carro = input("\nDigite o aluguel máximo desejado por dia (ou deixe em branco
para qualquer valor): ")
if aluguel_max_carro:
preferencias_usuario["aluguel_max"] = float(aluguel_max_carro)
resultados = recomendar_carro(preferencias_usuario)
if resultados:
print("\nCarros Disponíveis:")
for carro in resultados:
print(f"\n{carro.nome} - Tipo: {carro.tipo}, Consumo: {carro.consumo} km/l, Capacidade:
{carro.capacidade}, Aluguel/Dia: ${carro.aluguel:.2f}")
else:
